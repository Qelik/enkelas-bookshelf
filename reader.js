...
async function parseEpub(buf) {
    ...
    if (!containerFile)
        throw new Error("Not an ePub (no container.xml)");
    ...
    if (!rootEl)
        throw new Error("Broken ePub (no rootfile)");
    const opfPath = rootEl.getAttribute("full-path");
    const opfDir = dirOf(opfPath);
    const opfFile = zip.file(opfPath);
    if (!opfFile)
        throw new Error("Broken ePub (missing OPF)");
    ...
    if (!spine.length)
        throw new Error("Broken ePub (empty spine)");
...
let chapterBlobs = new Map();
function releaseChapterBlobs(i) {
...
function toggleDiag() {
    if (!els.diagEl)
        return;
    if (els.diagEl.hidden) {
        renderDiag();
        els.diagEl.hidden = false;
    }
    else
        els.diagEl.hidden = true;
}
// ---------------------------------------------------------------------------
// Chapter loading + sanitizing
// ---------------------------------------------------------------------------
async function chapterHTML(i) {
    if (chapterCache.has(i))
        return chapterCache.get(i);
    const path = book.spine[i];
    const f = book.zip.file(path);
    if (!f)
        return "<p>(Missing chapter)</p>";
...
function wrapTextRange(root, start, end, cls, hlId) {
    ...
    for (const nd of nodes) {
        ...
        if (a < b)
            segs.push({ nd, from: a - off, to: b - off });
        off += len;
        if (off >= end)
            break;
    }
    segs.reverse().forEach((sg) => {
        ...
        r.setStart(sg.nd, sg.from);
        r.setEnd(sg.nd, sg.to);
        const mark = document.createElement("mark");
        mark.className = cls;
        if (hlId)
            mark.dataset.hl = hlId;
        try {
            r.surroundContents(mark);
        }
        catch (e) { /* skip odd boundary */ }
    });
    return segs.length > 0;
}
function applyHighlightsTo(container, chIndex) {
    if (!rec || !rec.highlights || !rec.highlights.length)
        return;
    const hay = container.textContent;
    rec.highlights.filter((h) => h.ch === chIndex).forEach((h) => {
        const i = findOccurrence(hay, h.text, h.start);
        if (i >= 0)
            wrapTextRange(container, i, i + h.text.length, "rd-hl", h.id);
    });
}
...
function currentSelectionInfo() {
    ...
    if (!els.currentContent.contains(r.commonAncestorContainer))
        return null;
...
function hideSelbar() { if (els.selbar)
...
function maybeShowSelbar() {
    ...
    if (y < 44)
        y = info.rect.bottom + 8; // don't cover the top bar
...
function updateBookmarkBtn() {
    if (els.bookmarkBtn)
        els.bookmarkBtn.classList.toggle("on", !!findBookmarkHere());
}
...
async function gotoText(ch, text, near) {
    ...
    updateBars();
    saveProgress();
    setTimeout(() => { if (els && els.currentContent)
        removeFlashMarks(els.currentContent); }, 2200);
}
// ---------------------------------------------------------------------------
...
function openDrawer(tab) {
    ...
    renderDrawer();
    if (drawerTab === "search") {
        const inp = $("#rd-search-input");
        if (inp)
            setTimeout(() => inp.focus(), 60);
    }
}
function closeDrawer() { if (els.drawer)
    els.drawer.hidden = true; }
function renderDrawer() {
    if (!els.drawerBody || !book)
        return;
...
async function prepareTurn(dir) {
    ...
    let target;
    if (forward) {
        if (pos.page < pag.pages - 1)
            target = { ch: pos.ch, page: pos.page + 1 };
        else if (pos.ch < book.spine.length - 1)
            target = { ch: pos.ch + 1, page: 0 };
        else
            return null;
    }
    else {
        if (pos.page > 0)
            ...
            target = { ch: pos.ch - 1, page: -1 }; // -1 = last page
        else
            return null;
    }
...
function finishTurn(turn, committed) {
    return new Promise((resolve) => {
        ...
        const done = async () => {
            ...
            updateBars();
            saveProgress();
            if (committed && turn.forward)
                countPageRead(turn.prior);
            markActivity();
            turning = false;
            resolve();
        };
...
function setupDragTurn() {
    ...
    const move = (e) => {
        if (!drag || !drag.turn)
            return; // leaf not ready yet — ignore micro-moves
    ...
    const end = async (e) => {
        ...
        if (!turn) {
            turning = false;
            return;
        } // already at the cover / back page
    ...
    const capture = (z, e) => { try {
        z.setPointerCapture(e.pointerId);
    }
    catch (err) { /* pointer not active — drag still works via the window fallback */ } };
...
function countPageRead(prior) {
    if (!session || !book.chars || !book.chars.length)
        return;
...
function etaText() {
    if (!book || !book.chars || !book.chars.length)
        return "⏱ estimating…";
...
function startClock() {
    ...
    tickTimer = setInterval(() => {
        ...
        if (document.hidden)
            return; // not looking at the book
        if (now - session.lastActivity > IDLE_MS)
            return; // wandered off mid-page
        session.seconds += dt;
        ...
        if (session.seconds % 15 < dt)
            els.etaEl.textContent = etaText();
        if (session.seconds % 60 < dt) {
            updateSpeed();
            idbPut(rec);
            syncSessionLog();
        }
...
function syncSessionLog() {
    if (!session || session.seconds < 60)
        return;
    if (!rec || !rec.linkedBookId || !BookshelfAPI || !BookshelfAPI.upsertReadingLog)
        return;
...
async function openBook(id) {
    ...
    if (!r) {
        toast("⚠️", "Book not found", "Try uploading it again.");
        return;
    }
    try {
        grabEls();
...
function closeReader() {
    ...
    if (secs >= 60 && rec.linkedBookId && BookshelfAPI)
        syncSessionLog();
    idbPut(rec);
    session = null;
    releaseAllBlobs();
...
function showSessionSummary(s) {
    ...
    if (!modal || !body) {
        toast("📖", "Session saved", s.mins + " min of reading");
        return;
    }
    let streakRow = "";
    try {
        ...
        if (st && st.current > 1)
            streakRow = `<div class="rs-row"><span>🔥</span><span><strong>${st.current}-day</strong> reading streak${st.current >= st.longest ? " — your best!" : ""}</span></div>`;
    }
    catch (e) { /* ignore */ }
...
async function handleUpload(file) {
    ...
    try {
        ...
        if (existing) {
            toast("ℹ️", "Already in your eReader", (existing.meta && existing.meta.title) || file.name);
            return;
        }
...
function init() {
    ...
    if (!document.getElementById("ereader-upload-btn"))
        return;
    grabEls();
    ...
    $("#ereader-list").addEventListener("click", (e) => {
        ...
        if (e.target.closest(".er-read"))
            openBook(id);
        else if (e.target.closest(".er-delete")) {
            if (confirm("Remove this ePub from this device? Your bookshelf logs stay."))
                idbDel(id).then(renderList);
        }
    });
    ...
    if (els.diagEl)
        els.diagEl.addEventListener("click", (e) => { if (e.target.id === "reader-diag-close")
            els.diagEl.hidden = true; });
    ...
    els.drawerBody.addEventListener("click", async (e) => {
        ...
        if (del) {
            if (del.dataset.delbm)
                rec.bookmarks = (rec.bookmarks || []).filter((m) => m.id !== del.dataset.delbm);
        ...
        if (ch) {
            closeDrawer();
            await showChapter(Number(ch.dataset.goch), 0);
            markActivity();
            return;
        }
        ...
        if (hl) {
            ...
            if (h) {
                closeDrawer();
                await gotoText(h.ch, h.text, h.start);
                markActivity();
            }
            return;
        }
    ...
    $("#reader-eta").addEventListener("click", () => {
        ...
        try {
            localStorage.setItem(ETA_KEY, etaMode);
        }
        catch (e) { /* ignore */ }
        els.etaEl.textContent = etaText();
    });
    ...
    document.addEventListener("keydown", (e) => {
        ...
        if (e.key === "ArrowRight" || e.key === " ") {
            ...
            if (els.selbar && !els.selbar.hidden)
                hideSelbar();
    ...
    window.addEventListener("beforeunload", () => { if (rec) {
        updateSpeed();
        saveProgress();
        syncSessionLog();
    } });
