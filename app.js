...
try {
    SYNC_API = localStorage.getItem("enkelas-sync-api") || SYNC_API;
}
catch (e) { /* ignore */ }
...
const groupForView = (view) => NAV_GROUPS.find((g) => g.views.some((v) => v.view === view));
const lastViewPerGroup = {};
...
const PERF = (() => { try {
    return localStorage.getItem("enkelas-perf") === "1";
}
catch (e) {
    return false;
} })();
...
function hashHue(str) { let h = 0; for (let i = 0; i < str.length; i++)
    h = (h * 31 + str.charCodeAt(i)) >>> 0; return h % 360; }
// ---- ISBNs -----------------------------------------------------------------
...
function isbn13CheckDigit(body12) {
    let sum = 0;
    for (let i = 0; i < 12; i++)
...
function chipsHTML(book, clickable) {
    const t = (book.tags || []).map((x) => `<span class="tag${clickable ? " clickable" : ""}"${clickable ? ` data-tag="${esc(x)}"` : ""}>${esc(x)}</span>`);
    const c = (book.collections || []).map((x) => `<span class="tag coll${clickable ? " clickable" : ""}"${clickable ? ` data-collection="${esc(x)}"` : ""}>📁 ${esc(x)}</span>`);
...
function normalizeShelfObjects(raw) {
    if (!Array.isArray(raw))
        return [];
    return raw.reduce((out, o) => {
        if (!o || SHELF_OBJECT_KINDS.indexOf(String(o.kind)) === -1)
            return out;
...
function normalize(data) {
    ...
    base.books = Array.isArray(data.books) ? data.books.map((b) => ({
        ...
        finishHistory: Array.isArray(b.finishHistory) ? b.finishHistory.map((f) => typeof f === "string" ? { date: f, rating: null } : { date: f.date || null, rating: f.rating ? Number(f.rating) : null }) : [],
...
function saveAuth(a) { auth = a; try {
    a ? localStorage.setItem(AUTH_KEY, JSON.stringify(a)) : localStorage.removeItem(AUTH_KEY);
}
catch (e) { /* ignore */ } }
function loadSyncBase() { try {
    return localStorage.getItem(SYNCBASE_KEY) || null;
}
catch (e) {
    return null;
} }
function saveSyncBase(v) { try {
    v ? localStorage.setItem(SYNCBASE_KEY, v) : localStorage.removeItem(SYNCBASE_KEY);
}
catch (e) { /* ignore */ } }
...
function loadLastSync() { try {
    return localStorage.getItem(LASTSYNC_KEY) || null;
}
catch (e) {
    return null;
} }
function markSynced() { try {
    localStorage.setItem(LASTSYNC_KEY, new Date().toISOString());
}
catch (e) { /* ignore */ } }
function loadLastExport() { try {
    return localStorage.getItem(LASTEXPORT_KEY) || null;
}
catch (e) {
    return null;
} }
...
async function pushData(force) {
    ...
    if (force)
        ...
        body.baseUpdatedAt = loadSyncBase();
    try {
        ...
        if (res.status === 409 && data && data.blob) {
            ...
            if (useServer) {
                adoptServer(data.blob, data.updatedAt);
                markSynced();
                setSyncStatus("idle");
                return true;
            }
        ...
        return false; /* saved locally; retries on next change */
    }
}
async function pullData() {
    ...
    try {
        ...
        if (data.blob) {
            ...
            if (serverU > (state.updatedAt || ""))
                adoptServer(data.blob, serverU);
            else if ((state.updatedAt || "") > serverU) {
                await pushData(false);
                return;
            } // pushData sets status + markSynced
            else
                saveSyncBase(serverU);
        }
...
function setupAutoSync() {
    const maybePull = () => { if (syncEnabled() && auth && Date.now() - lastPullAt >= PULL_MIN_MS)
        pullData(); };
    ...
    window.addEventListener("online", () => { if (syncEnabled() && auth)
        pullData(); });
    window.addEventListener("offline", () => { if (syncEnabled() && auth)
        setSyncStatus("offline"); });
    setInterval(() => { if (!document.hidden)
        maybePull(); }, 15 * 60 * 1000);
}
...
async function logout() {
    closeAccountMenu();
    if (!confirm("Sign out on this device?\n\nYour books stay safely in your account, but they'll be removed from this device until you sign in again."))
        return;
    clearTimeout(pushTimer);
    const backedUp = await pushData(false);
    if (!backedUp && !confirm("Couldn't back up your latest changes — are you offline?\n\nSign out anyway? Changes made since the last sync will be lost."))
        return;
    saveAuth(null);
    saveSyncBase(null);
    try {
...
function closeAccountMenu() { const m = $("#account-menu"); if (m)
    m.hidden = true; }
...
function checkNewBadges() {
    ...
    if (knownBadges.size === 0) {
        badges.forEach((b) => { if (b.unlocked)
...
function computeChallenges() {
    ...
    const genresYr = new Set();
    finishedYr.forEach((b) => (b.tags || []).forEach((t) => genresYr.add(t.toLowerCase())));
    const chunky = state.books.some((b) => b.status === "finished" && b.totalPages >= 500);
    const decades = new Set();
    state.books.filter((b) => b.status === "finished" && b.publishedYear).forEach((b) => decades.add(Math.floor(b.publishedYear / 10)));
...
function authorMatches(query, names) {
    ...
    if (!q)
        return true; // no author given → nothing to disagree with
    ...
    return (names || []).some((n) => {
        ...
        if (qLast.length >= 3 && aTok.indexOf(qLast) >= 0)
            return true;
...
async function searchOpenLibrary(title, author, isbn) {
    ...
    if (isbn)
        params.set("isbn", isbn.replace(/[^0-9Xx]/g, ""));
    if (title)
    ...
    if (author)
        docs = docs.slice().sort((a, b) => (authorMatches(author, b.author_name) ? 1 : 0) - (authorMatches(author, a.author_name) ? 1 : 0));
    return docs;
}
...
async function findCoverFor(book) {
    for (const t of new Set([bareTitle(book), coreTitle(book)].filter(Boolean))) {
        try {
            ...
            if (hit)
                return coverFromId(hit.cover_i);
        }
        catch (e) { /* offline or OL hiccup — try the next form/source */ }
    }
    if (book.isbn && (await imgOk(coverFromIsbn(book.isbn))))
...
async function backfillCovers(force) {
    if (coverBackfillBusy || (coverBackfillRan && !force) || navigator.onLine === false)
        return;
    ...
    try {
        ...
        for (const b of state.books) {
            if (!force && b.coverTriedAt && Date.now() - new Date(b.coverTriedAt).getTime() < WEEK)
                continue;
            if (!b.coverUrl)
                todo.push(b);
            else if (isbnCover.test(b.coverUrl) && !(await imgOk(b.coverUrl)))
                todo.push(b);
        }
        let found = 0;
        for (const b of todo) {
        ...
        if (todo.length)
            commit();
        if (found)
            toast("🖼", "Covers found", found + " missing cover" + (found === 1 ? "" : "s") + " filled in");
    }
    finally {
...
function sessionInsights() {
    const logs = [];
    state.books.forEach((b) => (b.logs || []).forEach((l) => { if (l.date)
        logs.push({ date: l.date, pages: l.pages || 0, minutes: l.minutes || 0, format: b.format }); }));
    if (!logs.length)
        return null;
...
function tasteProfile() {
    ...
    const byAuthor = {};
    finished.forEach((b) => { if (b.rating && b.author)
        (byAuthor[b.author] = byAuthor[b.author] || []).push(b.rating); });
    let bestAuthor = null, bestAvg = 0;
    Object.keys(byAuthor).forEach((a) => { if (byAuthor[a].length >= 2) {
        const avg = byAuthor[a].reduce((s, r) => s + r, 0) / byAuthor[a].length;
        if (avg > bestAvg) {
            bestAvg = avg;
            bestAuthor = a;
        }
...
function coachNudges() {
    ...
    if (dnf.length >= 2)
        nudges.push(`You set aside <strong>${dnf.length}</strong> books this year — that's time saved for books you'll love, not something to feel bad about. ✨`);
    return nudges;
}
...
function shelfInsightLines() {
    ...
    if (openSeries)
        lines.push(`<strong>${openSeries}</strong> series waiting to be continued${ex ? ` — next up: <strong>${esc(ex.title)}</strong>` : ""}`);
    const dupes = duplicateGroups();
    if (dupes.length)
        lines.push(`<strong>${dupes.length}</strong> possible duplicate edition${dupes.length === 1 ? "" : "s"} — Shelf Doctor can merge them`);
    ...
    if (finished.length >= 3) {
        ...
        if (gap)
            lines.push(`You've finished <strong>${gap[1]} ${esc(gap[0])}</strong> books but have none waiting — a gap on your shelf`);
        // Author gap: someone you rate highly with nothing of theirs queued.
        const byAuthor = {};
        finished.forEach((b) => { const k = (b.author || "").trim(); if (k && b.rating) {
            (byAuthor[k] = byAuthor[k] || []).push(Number(b.rating));
        } });
        ...
        if (fav)
            lines.push(`You rate <strong>${esc(fav[0])}</strong> ${(fav[1].reduce((a, r) => a + r, 0) / fav[1].length).toFixed(1)}★ on average — nothing of theirs is on your list`);
    }
    return lines;
}
...
function moodMatchLine() {
    ...
    const recent = [];
    state.books.forEach((b) => (b.logs || []).forEach((l) => { if (new Date(l.date).getTime() >= cutoff && l.pages > 0)
        recent.push(l); }));
    if (recent.length < 3)
        return "";
...
function renderInsights() {
    ...
    if (shelf.length)
        cards.push(insightCard("🧭", "Shelf insights", shelf));
    if (mood)
        cards.push(insightCard("🎯", "For right now", [mood]));
    if (s)
        cards.push(insightCard("⏰", "Your reading rhythm", [
    ...
    if (t)
        cards.push(insightCard("🧬", "Your book DNA", [
    ...
    if (nudges.length)
        cards.push(`<div class="insight-card coach"><div class="insight-emoji">🌱</div><div><h4>A gentle nudge</h4>${nudges.map((n) => `<p>${n}</p>`).join("")}</div></div>`);
...
function readNextPicks(books) {
    ...
    if (want.length < 2 || finished.length < 2)
        return [];
    ...
    finished.forEach((b) => {
        ...
        if (ak) {
            const e = author[ak] = author[ak] || { sum: 0, n: 0, dnf: 0 };
            e.sum += w;
            e.n++;
        }
    });
    ...
    const picks = want.map((b) => {
        ...
        if (bestTag && bestTagScore >= 0.3)
            why.push(`you rate ${bestTag} highly`);
        ...
        if (ae) {
            ...
            score += s * 3;
            if (s >= 0.5)
                why.push(`more from ${b.author}`);
        }
        // Series: the very next installment is the strongest signal there is.
        if (b.seriesName) {
            const k = b.seriesName.toLowerCase();
            const done = seriesDone[k] || 0;
            if (done && b.seriesNumber != null && Number(b.seriesNumber) === Math.floor(done) + 1) {
                score += 4;
                why.push(`next in ${b.seriesName}`);
            }
            else if (done) {
                score += 0.5;
            }
        ...
        if (b.owned) {
            score += 1.5;
            why.push("already on your shelf");
        }
        if (b.expectation) {
            score += Number(b.expectation) - 3;
            if (Number(b.expectation) >= 4)
                why.push("you had high hopes for it");
        }
        const months = (now - new Date(b.addedAt || now).getTime()) / (30.44 * 24 * 3600 * 1000);
        if (months >= 6) {
            score += Math.min(2, months / 12);
            why.push(`${Math.round(months)} months on your list`);
        }
...
function libDate(b) {
    if (b.status === "dnf")
        return `<span class="dnf-badge" title="${b.dnfReason ? esc(b.dnfReason) : "Did not finish"}">Did not finish${b.dnfReason ? " · " + esc(b.dnfReason.length > 40 ? b.dnfReason.slice(0, 38) + "…" : b.dnfReason) : ""}</span>`;
...
function renderLibrary() {
    ...
    if (libraryTag && !tags.some((t) => t.toLowerCase() === libraryTag.toLowerCase()))
        libraryTag = "";
    ...
    if (libraryCollection && !cols.some((t) => t.toLowerCase() === libraryCollection.toLowerCase()))
        libraryCollection = "";
    ...
    if (libraryView === "shelf") {
        wrap.innerHTML = shelfHTML(list);
        if (list.length)
            empty.hidden = true;
    }
    else if (libraryView === "author")
        wrap.innerHTML = `<div class="author-view">${authorHTML(list)}</div>`;
    else if (libraryView === "series") {
        wrap.innerHTML = seriesHTML();
        empty.hidden = true;
    }
    else
        wrap.innerHTML = `<div class="card-grid library">${list.map(libraryCardHTML).join("")}</div>`;
}
...
const HOLD_SLOP = 10;
function holdMovedTooFar(startX, startY, x, y) {
...
function setupShelfDnD(root) {
    ...
    root.addEventListener("dragstart", (e) => {
        ...
        try {
            e.dataTransfer.setData("text/plain", slot.dataset.shelfId);
        }
    ...
    root.addEventListener("pointermove", (e) => {
        if (!touchDrag) {
            ...
            if (holdMovedTooFar(holdX, holdY, e.clientX, e.clientY))
                clearTimeout(holdTimer);
            return;
        }
        const el = document.elementFromPoint(e.clientX, e.clientY);
        const over = el && el.closest(".shelf-slot");
        if (!over || over === touchDrag || !over.parentNode)
            return;
        const r = over.getBoundingClientRect();
    ...
    root.addEventListener("click", (e) => { if (suppressClick) {
        e.stopPropagation();
        e.preventDefault();
    } }, true);
}
...
function seriesHTML() {
    ...
    if (!names.length)
        return `<p class="empty">No series yet — set “Series” on a book and your progress will show up here.</p>`;
...
function renderGoal() {
    ...
    if (target && done >= target)
        ...
        hint.textContent = `${target - done} to go in ${goal.year}.`;
    else
        hint.textContent = "";
    renderGoalExtra();
    renderChallenges();
...
function renderGoalExtra() {
    ...
    if (g.pagesTarget > 0) {
        ...
        html += metricHTML(`Pages in ${g.year}`, `${num(pr)} / ${num(g.pagesTarget)}`, pr / g.pagesTarget * 100, pr >= g.pagesTarget ? "🎉 Page goal reached!" : `${num(g.pagesTarget - pr)} pages to go`);
    }
    if (g.dailyPages > 0) {
        const tp = pagesOnDay(startOfDay(new Date()));
        html += metricHTML("Today's reading", `${num(tp)} / ${num(g.dailyPages)} pages`, tp / g.dailyPages * 100, tp >= g.dailyPages ? "✅ Daily goal done — nice!" : "Read a little more to hit today's goal.");
    }
    if (isThisYear && g.target > 0) {
        ...
        let pace;
        if (diff >= 0.1)
            pace = `🟢 You're ${rounded} book${rounded === 1 ? "" : "s"} ahead of schedule.`;
        else if (diff <= -0.1)
            pace = `🟠 You're ${rounded} book${rounded === 1 ? "" : "s"} behind schedule.`;
        else
            pace = `🎯 Right on pace.`;
...
function monthlyItems(logs) {
    ...
    logs.forEach((x) => { const b = buckets.find((bk) => bk.y === x.d.getFullYear() && bk.m === x.d.getMonth()); if (b)
        b.value += x.pages; });
...
function ratingItems() {
    ...
    state.books.forEach((b) => { const r = Math.round(b.rating || 0); if (r >= 1 && r <= 5)
        counts[r - 1]++; });
...
function histPushBook(id) {
    try {
        history.pushState({ bookId: id, fromApp: true }, "", "#book/" + id);
        return true;
    }
    catch (e) {
        return false;
...
function openBookPage(book, opts) {
    ...
    if (book.seriesName)
        meta.push(`📚 ${esc(book.seriesName)}${book.seriesNumber ? " #" + book.seriesNumber : ""}`);
    meta.push(`${num(read)}${book.totalPages ? " / " + num(book.totalPages) : ""} ${unitLabel(book)}${book.totalPages ? " · " + pct + "%" : ""}`);
    if (book.publishedYear)
        meta.push(`First published ${book.publishedYear}`);
    if (dates)
        meta.push(dates);
    if (book.readCount > 1)
        meta.push(`Read ${book.readCount}×`);
    ...
    if (book.owned)
        meta.push(`🏠 On your home shelf`);
    if (book.lentTo)
        meta.push(`📤 With ${esc(book.lentTo)}${book.lentAt ? " since " + fmtDate(book.lentAt) : ""}`);
    if (book.loanDue)
        meta.push(loanBadgeHTML(book));
    if (book.bookmark)
    ...
    if (activeView !== "book") {
        bookReturnView = activeView;
        bookReturnScroll = window.scrollY;
    }
    switchView("book");
    if (!opts.keepScroll)
        window.scrollTo(0, 0);
    if (opts.push !== false) {
        const st = histState();
        if (!(st && st.bookId === book.id))
            histPushBook(book.id);
    }
}
...
function refreshDetail(force) {
    ...
    if (b)
        openBookPage(b, { push: false, keepScroll: true });
}
...
function svgProgress(book) {
    ...
    if (distinctDays < 2)
        return progressMeter(book, logs);
...
function openYearReview(year) {
    ...
    finished.forEach((b) => monthCounts[new Date(b.finishedAt).getMonth()]++);
...
function bookMarkdown(b) {
    ...
    const meta = [];
    if (b.rating)
        meta.push(`Rating: ${fmtRating(b.rating)}★`);
    if (b.expectation)
        meta.push(`Expected: ${fmtRating(b.expectation)}★`);
    if (b.status === "finished" && b.finishedAt)
        meta.push(`Finished: ${fmtDate(b.finishedAt)}`);
    if (b.readCount > 1)
        meta.push(`Read ${b.readCount}×`);
    if (b.totalPages)
        meta.push(`${num(b.totalPages)} ${unitLabel(b)}`);
    if (b.seriesName)
        meta.push(`${b.seriesName}${b.seriesNumber ? " #" + b.seriesNumber : ""}`);
    if (meta.length)
        lines.push(meta.join(" · "), "");
    if (b.pickReason)
        lines.push(`## Why I picked it up`, "", b.pickReason, "");
    if (b.review)
        lines.push(`## My review`, "", b.review, "");
    if (b.dnfReason)
...
function wrapText(ctx, text, x, y, maxW, lh, maxLines) {
    ...
    for (let i = 0; i < words.length; i++) {
        ...
        if (ctx.measureText(test).width > maxW && line) {
            if (lines + 1 >= maxLines) {
                ctx.fillText(line.replace(/.{3}$/, "") + "…", x, y);
                return y + lh;
            }
...
async function shareBookCard(b) {
    ...
    g.addColorStop(1, "#efe2ca");
    ctx.fillStyle = g;
    ctx.fillRect(0, 0, W, H);
    ctx.strokeStyle = "#c98a4b";
    ctx.lineWidth = 5;
    ctx.strokeRect(42, 42, W - 84, H - 84);
    ctx.strokeStyle = "rgba(201,138,75,.45)";
    ctx.lineWidth = 1.5;
    ...
    if (b.coverUrl) {
        try {
            ...
            ctx.save();
            ctx.shadowColor = "rgba(60,40,20,.4)";
            ctx.shadowBlur = 34;
    ...
    if (b.author) {
        ...
        ctx.fillText("by " + b.author, W / 2, y + 8);
        y += 56;
    }
    if (b.rating) {
        ctx.textAlign = "left";
    ...
    const bits = [];
    if (b.status === "finished" && b.finishedAt)
        bits.push("Finished " + fmtDate(b.finishedAt));
    if (b.totalPages)
        bits.push(num(b.totalPages) + " " + unitLabel(b));
    if (b.readCount > 1)
        bits.push("read " + b.readCount + "×");
    if (bits.length) {
        ctx.fillText(bits.join("  ·  "), W / 2, y + 16);
        y += 60;
    }
...
async function shareSnapshotCard() {
    ...
    ctx.fillText("📚 " + appTitle(), W / 2, 135);
    ctx.fillStyle = "#6b6258";
    ctx.font = "italic 30px Georgia, serif";
    ctx.fillText("My reading, right now", W / 2, 185);
    const streak = readingStreak();
    const stats = [
    ...
    stats.forEach((s, i) => {
        ...
        ctx.fillText(s[0], cxp, cyp);
        ctx.fillStyle = "#6b6258";
        ctx.font = "26px Georgia, serif";
        ctx.fillText(s[1], cxp, cyp + 42);
    });
    const genre = {};
    booksFinished().forEach((b) => (b.tags || []).forEach((t) => { genre[t] = (genre[t] || 0) + 1; }));
    const top = Object.keys(genre).sort((a, b) => genre[b] - genre[a])[0];
    if (top) {
        ctx.fillStyle = "#4a4238";
        ctx.font = "italic 30px Georgia, serif";
        ctx.fillText("Favourite genre: " + top, W / 2, H - 135);
    }
    ctx.fillStyle = "#9c5b3a";
    ctx.font = "600 24px Georgia, serif";
    ctx.fillText(new Date().toLocaleDateString(undefined, { month: "long", day: "numeric", year: "numeric" }), W / 2, H - 85);
    ctx.textAlign = "left";
    try {
        cv.toBlob((blob) => {
            if (!blob) {
...
async function shareYearCard(year) {
    ...
    for (let i = 0; i < 14; i++) {
        const bw = 46 + (i * 37) % 40;
        ctx.fillRect(60 + i * 70, H - 210, bw * 0.55, 150 + (i * 53) % 50);
    }
    ctx.fillStyle = "#fff";
    ctx.textAlign = "center";
...
function giftListUrl(books) {
    if (!/^https?:/.test(location.protocol))
        return ""; // file:// has no shareable origin
    const payload = { v: 1, n: sharerName(), b: books.map((b) => [b.title, b.author || "", b.isbn || ""]) };
...
function drawGiftCard(books) {
    ...
    ctx.fillText("🎁 Books I'd love", W / 2, 150);
    ctx.fillText("to be gifted", W / 2, 218);
    ctx.fillStyle = "#6b6258";
    ctx.font = "italic 30px Georgia, serif";
    ctx.fillText(sharerName() + "'s reading wishlist", W / 2, 268);
    ctx.textAlign = "left";
    shown.forEach((b, i) => {
        ...
        let ty = y + 34;
        ctx.fillText(clampText(ctx, b.title, maxW), tx, ty);
        ty += 38;
        if (b.author) {
            ctx.fillStyle = "#6b6258";
            ctx.font = "italic 26px Georgia, serif";
            ctx.fillText(clampText(ctx, "by " + b.author, maxW), tx, ty);
        }
        ctx.strokeStyle = "rgba(201,138,75,.25)";
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(marginX, y + rowH - 14);
        ctx.lineTo(W - marginX, y + rowH - 14);
        ctx.stroke();
    });
    ctx.textAlign = "center";
    ctx.fillStyle = "#6b6258";
    ctx.font = "italic 26px Georgia, serif";
    if (extra > 0)
        ctx.fillText("…and " + extra + " more", W / 2, H - footerH + 60);
    ctx.fillStyle = "#9c5b3a";
    ctx.font = "600 26px Georgia, serif";
...
async function shareGiftListImage() {
    ...
    if (!books.length) {
        toast("📋", "Pick at least one book", "Tick the books you'd like to include.");
        return;
    }
    const blob = await toBlobAsync(drawGiftCard(books));
    if (!blob) {
        toast("⚠️", "Couldn't create the image", "");
        return;
    }
    ...
    if (navigator.canShare && navigator.canShare({ files: [file] })) {
        try {
            await navigator.share({ files: [file], title: "Books I'd love to be gifted", text });
            return;
        }
        catch (e) {
            if (e && e.name === "AbortError")
                return; /* fall through to download */
        }
    }
...
async function shareGiftListLink() {
    ...
    if (url && navigator.share) {
        try {
            await navigator.share({ title: "Books I'd love to be gifted", text, url });
            return;
        }
        catch (e) {
            if (e && e.name === "AbortError")
                return; /* fall through to clipboard */
        }
    }
    const body = url ? text + "\n" + url
    ...
    try {
        ...
        toast("📋", "Wishlist copied", url ? "Send the link to a friend." : "Paste it to a friend.");
    }
    catch (e) {
        toast("ℹ️", "Your wishlist", body);
    }
}
// Buy-search links a gift-giver can act on — ISBN when we have it, else title+author.
function buyLinks(title, author, isbn) {
...
function openReceivedGiftList(payload) {
...
function maybeOpenGiftLink() {
    ...
    try {
        ...
        if (payload && Array.isArray(payload.b))
            openReceivedGiftList(payload);
    }
    catch (e) {
        toast("⚠️", "Couldn't open that wishlist", "The link looks incomplete.");
    }
}
// --- Monthly wrap-up ---
...
function maybeShowMonthlyRecap() {
    ...
    let lastSeen = null;
    try {
        lastSeen = localStorage.getItem("enkelas-last-recap");
    }
    catch (e) { /* ignore */ }
    try {
        localStorage.setItem("enkelas-last-recap", curMonth);
    }
    catch (e) { /* ignore */ }
    if (!lastSeen || lastSeen === curMonth)
        return;
    const prev = new Date();
    prev.setDate(1);
...
function journeyEventsUncached() {
    ...
    state.books.forEach((b) => {
        if (b.addedAt)
            ev.push({ t: b.addedAt, icon: "➕", title: `Added “${b.title}”`, sub: b.pickReason ? "💭 " + b.pickReason : "", id: b.id });
        if (b.startedAt && b.status !== "want")
            ev.push({ t: b.startedAt, icon: "▶️", title: `Started “${b.title}”`, sub: "", id: b.id });
        ...
        (b.quotes || []).forEach((q) => { if (q.at)
            ev.push({ t: q.at, icon: "❝", title: `Saved a quote from “${b.title}”`, sub: "“" + q.text + "”", id: b.id }); });
        ...
        if (b.status === "finished" && b.finishedAt && !history.length)
            ev.push({ t: b.finishedAt, icon: "🏁", title: `Finished “${b.title}”`, sub: b.rating ? "Rated " + fmtRating(b.rating) + "★" : "", id: b.id });
...
function renderOwned() {
    ...
    if (locSel) {
        ...
        if (ownedLocation && !locs.some((l) => l.toLowerCase() === ownedLocation.toLowerCase()))
            ownedLocation = "";
    ...
    const parts = [];
    if (fmtCounts.physical)
        parts.push(`${num(fmtCounts.physical)} physical`);
    if (fmtCounts.ebook)
        parts.push(`${num(fmtCounts.ebook)} e-book${fmtCounts.ebook === 1 ? "" : "s"}`);
    if (fmtCounts.audio)
        parts.push(`${num(fmtCounts.audio)} audiobook${fmtCounts.audio === 1 ? "" : "s"}`);
    if (unreadCount)
        parts.push(`${num(unreadCount)} unread`);
...
function renderStorageStatus() {
    ...
    if (syncEnabled() && auth && auth.user) {
        ...
        if (syncStatus === "syncing") {
            txt = "☁️ Syncing…";
            note = "Saving your latest changes to your account.";
        }
        else if (syncStatus === "offline") {
            txt = "📴 Offline";
            note = "You're offline — changes are saved on this device and will sync when you're back.";
        }
        else if (syncStatus === "error") {
            txt = "⚠️ Sync paused";
            note = "Couldn't reach your account — changes are safe on this device and will retry.";
        }
        else {
            txt = "☁️ Synced" + (last ? " · " + last : "");
            note = "Your books sync privately to your account (" + auth.user.email + ").";
        }
        el.textContent = txt;
    ...
    if (fileHandle) {
        ...
        el.title = "Changes are written to your connected JSON file. Tap for settings.";
        return;
    }
...
function syncBadgeText() {
    ...
    if (!auth || !auth.user)
        return "💾 Saved on this device (not signed in)";
    if (syncStatus === "syncing")
        return "☁️ Syncing…";
    if (syncStatus === "offline")
        return "📴 Offline — saved on this device";
    if (syncStatus === "error")
        return "⚠️ Sync paused — saved on this device";
    if (syncStatus === "needslogin")
...
function renderBackupHealth() {
    ...
    if (navigator.storage && navigator.storage.persisted) {
        navigator.storage.persisted().then((p) => {
            const el = $("#bh-persist");
            if (el)
                el.innerHTML = `<span>${p ? "🔒" : "⚠️"}</span><span>${p ? "The browser granted persistent storage — it won't auto-clear your data." : "Storage isn't marked persistent yet — installing the app (Add to Home Screen) protects it."}</span>`;
        }).catch(() => { const el = $("#bh-persist"); if (el)
            el.hidden = true; });
    }
    else {
        const el = $("#bh-persist");
...
function maybeBackupReminder() {
    try {
        if (state.books.length < 5)
            return;
        const now = Date.now();
        if (now - Number(localStorage.getItem(BACKUPNAG_KEY) || 0) < 13 * DAY)
            return;
        const lastX = loadLastExport();
        const ageDays = lastX ? (now - new Date(lastX).getTime()) / DAY : Infinity;
        if (ageDays < ((syncEnabled() && auth) ? 60 : 30))
            return;
...
async function refreshAppFiles() {
    if (!confirm("Refresh the app's files?\n\nThis clears the cached app and reloads the latest version. Your books are safe."))
        return;
    try {
        if (window.caches) {
...
async function fetchGenresForBook(book) {
    ...
    if (book.author && !book.isbn) {
        const m = docs.filter((d) => authorMatches(book.author, d.author_name));
        if (m.length)
            docs = m;
    }
    ...
    if (picks.length < 3 && doc && doc.key) {
        try {
            ...
            picks = cleanSubjects((w.subjects || []).concat(doc.subject || []));
        }
...
function mergeDuplicateGroup(group) {
    ...
    others.forEach((o) => {
        ...
        ["coverUrl", "author", "isbn", "description", "review", "seriesName", "dnfReason", "pickReason", "lentTo"].forEach((f) => { if (!primary[f] && o[f])
            primary[f] = o[f]; });
        if (!primary.totalPages && o.totalPages)
            primary.totalPages = o.totalPages;
        if (primary.seriesNumber == null && o.seriesNumber != null)
            primary.seriesNumber = o.seriesNumber;
        if (!primary.rating && o.rating)
            primary.rating = o.rating;
        if (!primary.finishedAt && o.finishedAt)
            primary.finishedAt = o.finishedAt;
        if (!primary.startedAt && o.startedAt)
            primary.startedAt = o.startedAt;
    });
...
function renderShelfDoctor() {
    ...
    if (!state.books.length) {
        body.innerHTML = `<p class="doctor-clean">Add some books first, then Shelf Doctor will help you tidy them up.</p>`;
        return;
    }
    if (!issues.length) {
        body.innerHTML = `<p class="doctor-clean">🎉 Your shelves look healthy — nothing needs attention.</p>`;
        return;
    }
...
function closeClubWs() { if (clubSocket) {
    try {
        clubSocket.onclose = null;
        clubSocket.close();
    }
...
function markClubSeen(clubId) { try {
    const s = loadClubSeen();
    s[clubId] = new Date().toISOString();
    localStorage.setItem("enkelas-club-seen", JSON.stringify(s));
}
catch (e) { /* ignore */ } }
...
function openClubsShell() {
    closeAccountMenu();
    if (!syncEnabled()) {
        toast("ℹ️", "Clubs need an account", "This feature syncs with friends via your account.");
        return false;
    }
    if (!auth) {
        closeModals();
        openAuthModal("login");
        toast("👤", "Sign in first", "Reading clubs sync with friends through your account.");
        return false;
    }
    currentClubId = null;
...
async function joinClubByCode(code) {
    ...
    if (body)
        body.innerHTML = `<p class="muted">Joining club…</p>`;
    try {
        ...
        if (res.ok && data.clubId) {
            toast("👥", "Joined the club!", "");
            openClub(data.clubId);
            return;
        }
...
async function shareClubInvite(code, bookTitle) {
    ...
    if (navigator.share) {
        try {
            await navigator.share(url ? { title: "Reading club invite", text, url } : { title: "Reading club invite", text });
            return;
        }
        catch (e) {
            if (e && e.name === "AbortError")
...
async function renderClubsListScreen() {
    ...
    if (res.status === 503) {
        body.innerHTML = `<p class="empty">Reading clubs aren't switched on yet — the sync worker needs its clubs database enabled.</p>`;
        return;
    }
    if (!res.ok) {
        body.innerHTML = `<p class="empty">Couldn't load your clubs — check your connection.</p>`;
        return;
    }
...
async function openClub(clubId) {
    ...
    const loop = () => { clubPollTimer = setTimeout(async () => { if (currentClubId === clubId && !$("#clubs-modal").hidden) {
        if (!clubSocket)
...
async function refreshClub(quiet) {
    ...
    if (!detailR.res.ok) {
        if (!quiet)
            $("#clubs-body").innerHTML = `<p class="empty">Couldn't open this club.</p><button class="mini" data-club-back>← All clubs</button>`;
        return;
    }
    // Don't clobber the box the user is typing in on a background poll.
    if (quiet && document.activeElement && document.activeElement.id === "club-comment-body")
        return;
    renderClubScreen(detailR.data, commentsR.data || {});
}
function renderClubScreen(d, cm) {
    const body = $("#clubs-body");
    if (!body || !d.club || currentClubId !== d.club.id)
        return;
    const me = d.me || {};
...
async function toggleReaction(commentId, emoji) {
    if (!currentClubId || !commentId || !emoji)
        return;
...
async function postClubComment() {
    ...
    if (!res.ok) {
        toast("⚠️", "Couldn't post", "Try again in a moment.");
        if (el)
            el.value = bodyText;
        return;
    }
...
// ---------------------------------------------------------------------------
let communityCategory = "", communitySort = "top", communityHideRead = true, lastRecs = null, recsSignedIn = false, recsCapped = false;
...
function isbnDigits(s) { return canonicalISBN(s); }
...
function readMatchers() {
    ...
    booksFinished().forEach((b) => {
        const t = normStr(b.title);
        if (t) {
            titles.add(t);
            pairs.add(t + "|" + normStr(b.author));
        }
        const i = isbnDigits(b.isbn);
        if (i.length >= 10)
            isbns.add(i);
    });
...
function recIsRead(r, m) {
    const i = isbnDigits(r.book_isbn);
    if (i.length >= 10 && m.isbns.has(i))
        return true;
    const t = normStr(r.book_title);
    if (!t)
        return false;
...
async function renderCommunity() {
    ...
    if (!body)
        return;
    if (!syncEnabled()) {
        body.innerHTML = `<p class="empty">Community recommendations sync through the app's account server, which isn't configured here.</p>`;
        return;
    }
    if (lastRecs === null)
        body.innerHTML = `<p class="muted">Loading recommendations…</p>`;
    const { res, data } = await recsApi("");
    if (res.status === 503) {
        body.innerHTML = `<p class="empty">Recommendations aren't switched on yet — the sync worker needs its database enabled.</p>`;
        return;
    }
    if (!res.ok || !data) {
        body.innerHTML = `<p class="empty">Couldn't load recommendations — check your connection.</p>`;
        return;
    }
    lastRecs = data.recs || [];
...
function drawCommunity() {
    ...
    let recs = all.filter((r) => {
        if (matchers && recIsRead(r, matchers)) {
            hiddenRead++;
            return false;
        }
        if (communityCategory && normStr(r.category) !== normStr(communityCategory))
            return false;
        return true;
    });
...
function populateCommunityCategoryFilter(recs) {
    ...
    if (cur && !cats.some((c) => normStr(c) === normStr(cur))) {
        communityCategory = "";
        sel.value = "";
    }
}
async function voteRec(id, vote) {
    ...
    if (!auth) {
        closeModals();
        openAuthModal("login");
        toast("👤", "Sign in to vote", "Voting is tied to your account so everyone votes once.");
        return;
    }
    ...
    if (!res.ok) {
        toast("⚠️", "Couldn't record your vote", "Try again in a moment.");
        await renderCommunity();
        return;
    }
    if (r && data && typeof data.myVote === "number") {
        r.myVote = data.myVote;
        r.score = r.up - r.down;
    }
}
async function deleteRec(id) {
    if (!id || !confirm("Remove your recommendation from the community board?"))
        return;
    const { res } = await recsApi("/" + id + "/delete", { method: "POST", body: "{}" });
    if (!res.ok) {
        toast("⚠️", "Couldn't remove it", "Try again in a moment.");
        return;
    }
...
function openRecommendModal(prefill) {
    if (!syncEnabled()) {
        toast("ℹ️", "Needs an account", "Recommendations sync through your account.");
        return;
    }
    if (!auth) {
        closeModals();
        openAuthModal("login");
        toast("👤", "Sign in first", "Recommending a book is tied to your account.");
        return;
    }
    ...
    setTimeout(() => { const t = $("#rec-title"); if (t)
        t.focus(); }, 60);
}
async function submitRecommend() {
    ...
    if (!title) {
        toast("✍️", "Add a title", "Which book are you recommending?");
        return;
    }
    if (!category) {
        toast("🏷️", "Pick a category", "Which shelf does it belong on?");
        return;
    }
    ...
    if (!cover || !author) {
        ...
        if (match) {
            author = author || match.author || "";
            cover = cover || match.coverUrl || "";
            isbn = isbn || match.isbn || "";
        }
    }
    ...
    if (!res.ok) {
        toast("⚠️", "Couldn't share it", "Try again in a moment.");
        return;
    }
    closeModals();
...
function updateInstallUI() {
    ...
    const b1 = $("#btn-install-app");
    if (b1)
        b1.hidden = !has;
    const b2 = $("#onboard-install");
    if (b2)
        b2.hidden = !has;
}
async function promptInstall() {
    if (!deferredInstallPrompt) {
        toast("ℹ️", "Add to Home Screen", "Use your browser's Share menu → “Add to Home Screen.”");
        return;
    }
...
function maybeShowOnboarding() {
    let done = false;
    try {
        done = localStorage.getItem(ONBOARD_KEY) === "1";
    }
    catch (e) { /* ignore */ }
    if (done)
        return;
    ...
    if (state.books.length > 0 || (syncEnabled() && auth)) {
        finishOnboarding();
        return;
    }
    updateInstallUI();
...
function openBookModal(opts) {
    ...
    if (book && (!book.tags || !book.tags.length))
        fetchGenresForForm({ quiet: true, onlyIfEmpty: true, forId: book.id });
}
...
function cleanSubjects(subjects) {
    ...
    for (let s of subjects) {
        s = String(s || "").trim();
        if (s.length < 3 || s.length > 26 || BAD.test(s))
            continue;
...
async function fetchGenresForForm(opts) {
    ...
    if (!title && !isbn) {
        if (!opts.quiet)
            toast("ℹ️", "Add a title first", "Then I can look up its genres.");
        return;
    }
    ...
    try {
        ...
        if (author && !isbn) {
            const m = docs.filter((d) => authorMatches(author, d.author_name));
            if (m.length)
                docs = m;
        }
        const doc = docs.find((d) => Array.isArray(d.subject) && d.subject.length) || docs[0];
        ...
        if (opts.forId && $("#f-id").value !== opts.forId)
            return; // modal moved on to another book
        if (!picks.length) {
            if (!opts.quiet)
                toast("🔍", "No genres found", "Add a couple by hand below.");
            return;
        }
        const existing = parseList($("#f-tags").value);
        if (opts.onlyIfEmpty && existing.length)
            return;
        const have = new Set(existing.map((x) => x.toLowerCase()));
        const merged = existing.slice();
        picks.forEach((pk) => { if (!have.has(pk.toLowerCase())) {
            have.add(pk.toLowerCase());
            merged.push(pk);
        } });
        $("#f-tags").value = merged.join(", ");
        renderTagHelpers();
        if (!opts.quiet)
            toast("🏷️", "Genres added", picks.join(", "));
    }
    catch (e) {
        if (!opts.quiet)
            toast("⚠️", "Couldn't fetch genres", "Check your connection, or add them by hand.");
    }
    finally {
        if (btn && !opts.quiet) {
            btn.disabled = false;
            btn.textContent = "✨ Fetch genres";
        }
    }
}
async function handleFetch() {
    const title = $("#f-title").value.trim(), author = $("#f-author").value.trim(), isbn = $("#f-isbn").value.trim();
    if (!title && !isbn) {
        toast("ℹ️", "Type a title first", "Then I can look up the cover.");
        return;
    }
    const btn = $("#btn-fetch");
    btn.disabled = true;
    ...
    try {
        ...
        if (!docs.length) {
            toast("🔍", "No matches found", author ? "Double-check the author's spelling, or paste a cover URL." : "Try adding the author, or paste a cover URL.");
            return;
        }
        ...
        btn.disabled = false;
        btn.textContent = "🔍 Auto-fetch details & cover";
    }
}
...
async function applyDoc(doc, opts) {
    ...
    if (cover && (overwrite || !$("#f-cover").value)) {
        $("#f-cover").value = cover;
    ...
    if (doc.key && (overwrite || picks.length < 3 || !$("#f-desc").value.trim())) {
        try {
            ...
            d = d ? String(d).split("\n")[0].slice(0, 600) : "";
            if (overwrite)
                $("#f-desc").value = d; // reflect the chosen book, even if blank
            else if (d && !$("#f-desc").value.trim())
                $("#f-desc").value = d;
            const richer = cleanSubjects((w.subjects || []).concat(doc.subject || []));
            if (richer.length > picks.length && (overwrite || !$("#f-tags").value.trim())) {
                $("#f-tags").value = richer.join(", ");
                renderTagHelpers();
            }
        }
...
function updateLogPageHint() {
    ...
    if ($("#log-pages").value !== "") {
        ...
        if (delta > 0)
            msg += ` That's +${num(delta)} ${unitLabel(book)} this session.`;
        else {
            msg += " ⚠️ Enter a page beyond where you left off.";
            warn = true;
        }
    }
...
function eanMatchDigit(runs, i, rightSide) {
    ...
    if (!best || bestD > 1.5 || secondD - bestD < 0.25)
        return null; // too blurry/ambiguous
    return best;
}
function eanGuardOk(runs, i, count, m) {
    ...
    if (Math.abs(total / count - m) > m * 0.55)
        return false;
    for (let k = 0; k < count; k++)
        if (Math.abs(runs[i + k] - m) > m * 0.7)
            return false;
    return true;
}
function eanDecodeRuns(runs, startsDark) {
    ...
    for (let i = startsDark ? 0 : 1; i + 58 < runs.length; i += 2) {
        ...
        if (!ok || !eanGuardOk(runs, i + 27, 5, m))
            continue;
        for (let d = 0; d < 6 && ok; d++) {
            const g = i + 32 + d * 4;
            const tot = runs[g] + runs[g + 1] + runs[g + 2] + runs[g + 3];
        ...
        if (!ok || !eanGuardOk(runs, i + 56, 3, m))
            continue;
        ...
        let sum = 0;
        for (let k = 0; k < 12; k++)
            sum += Number(code[k]) * (k % 2 ? 3 : 1);
        if ((10 - (sum % 10)) % 10 !== Number(code[12]))
            continue;
        if (!/^97[89]/.test(code))
            continue; // books only — ignores price add-on codes
        return code;
    }
    return null;
}
function eanRowRuns(data, w, y) {
    ...
    if (max - min < 48)
        return null; // no barcode contrast on this line
    const thr = (min + max) / 2;
    const runs = [];
    let dark = lum[0] < thr, len = 1;
    for (let x = 1; x < w; x++) {
        const d = lum[x] < thr;
        if (d === dark)
            len++;
...
async function openScan(onDetect) {
    ...
    if ("BarcodeDetector" in window) {
        try {
            scanDetector = new window.BarcodeDetector({ formats: ["ean_13", "ean_8", "upc_a"] });
        }
        catch (e) {
            try {
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        toast("⚠️", "No camera access", "Type the ISBN instead.");
        return;
    }
    try {
        ...
        toast("⚠️", "Camera blocked", "Allow camera access, or type the ISBN.");
        return;
    }
    const v = $("#scan-video");
        ...
        try {
            ...
            if (scanDetector) {
                ...
                if (codes && codes.length)
                    raw = String(codes[0].rawValue || "").replace(/[^0-9Xx]/g, "");
            }
            else {
...
function seriesInsight(seriesName, thisNumber) {
    ...
    if (thisNumber != null) {
        if (inSeries.some((b) => b.seriesNumber === thisNumber && b.owned))
            parts.push(`You already own #${thisNumber}.`);
        else
            parts.push(`This is #${thisNumber}.`);
        if (inSeries.some((b) => b.seriesNumber === thisNumber - 1 && b.status !== "finished"))
            parts.push(`Heads up — you haven't read #${thisNumber - 1} yet.`);
    }
    return parts.join(" ");
}
async function checkScannedBook(isbn) {
    ...
    if (!hit) {
        try {
            ...
            if (docs[0]) {
                scanTitle = docs[0].title || "";
                scanAuthor = (docs[0].author_name || [])[0] || "";
            }
        }
    ...
    if (hit && hit.owned) {
        const extra = [];
        if (hit.format && hit.format !== "physical")
            extra.push(`You have this as ${fmtName[hit.format]}.`);
        if (hit.lentTo)
            extra.push(`📤 But it's lent to ${esc(hit.lentTo)} right now.`);
...
function wireStars(container, onSet) {
    container.addEventListener("click", (e) => { const s = e.target.closest("[data-star]"); if (!s)
        return; const v = starValueFromEvent(e, s); onSet(v); paintStars(container, v); });
    container.addEventListener("mousemove", (e) => { const s = e.target.closest("[data-star]"); if (!s)
        return; paintStarSpans(container, starValueFromEvent(e, s)); });
    ...
    reader.onload = async () => {
        try {
            ...
            if (isFull && Array.isArray(data.epubs) && data.epubs.length && EReader && EReader.importAll) {
                try {
                    await EReader.importAll(data.epubs.map((r) => Object.assign({}, r, { data: b64ToBuf(r.data) })));
                    epubNote = " · " + data.epubs.length + " ePub" + (data.epubs.length === 1 ? "" : "s") + " restored";
                }
                catch (e2) {
                    epubNote = " · ePubs couldn't be restored";
                }
            }
            ...
            toast("⚠️", "Import failed", "That file isn't valid bookshelf JSON.");
        }
    };
...
function importGoodreads(file) {
    ...
    reader.onload = () => {
        try {
            ...
            const cTitle = idx("Title"), cAuthor = idx("Author"), cRating = idx("My Rating"), cPages = idx("Number of Pages"), cShelf = idx("Exclusive Shelf"), cShelves = idx("Bookshelves"), cReview = idx("My Review"), cDateRead = idx("Date Read"), cISBN = idx("ISBN"), cISBN13 = idx("ISBN13"), cYear = idx("Original Publication Year"), cOwned = idx("Owned Copies");
            if (cTitle < 0)
                throw 0;
            const clean = (s) => {
            ...
            console.warn(e);
            toast("⚠️", "Import failed", "That doesn't look like a Goodreads CSV export.");
        }
    };
    reader.readAsText(file);
}
...
function renderBottomNav(activeGroup) {
...
function onMainClick(e) {
    ...
    if (actBtn) {
        ...
        if (action === "dismiss-check") {
            $("#owned-check-result").hidden = true;
            return;
        }
        if (action === "scan-add") {
            openBookModal({ status: "want" });
            $("#f-isbn").value = actBtn.dataset.isbn || "";
            if ($("#f-isbn").value)
                handleFetch();
            return;
        }
        ...
        if (action === "log")
            ...
            commit();
            toast("📖", "Started reading", book.title);
        }
            ...
            commit();
            toast("🏠", book.owned ? "Added to your home shelf" : "Removed from your home shelf", book.title);
        }
        else if (action === "dnf")
            openDnfModal(book);
    ...
    if (tagChip) {
        ...
        if (activeView === "library") {
            libraryTag = tag;
            $("#library-tag").value = tag;
            renderLibrary();
        }
        else if (activeView === "want") {
            wantQuery = tag;
            $("#want-search").value = tag;
            renderWant();
        }
        else {
            readingQuery = tag;
            $("#reading-search").value = tag;
            renderReading();
        }
        return;
    }
    ...
    if (addBtn) {
        openBookModal({ status: addBtn.dataset.add });
        return;
    }
...
function init() {
    ...
    $("#library-view-toggle").addEventListener("click", (e) => { const b = e.target.closest("[data-libview]"); if (b) {
        libraryView = b.dataset.libview;
        renderLibrary();
    } });
    // Genre + collection quick-add chips
    $("#tag-suggest").addEventListener("click", (e) => { const chip = e.target.closest("[data-add-tag]"); if (!chip)
        return; const t = parseList($("#f-tags").value); t.push(chip.dataset.addTag); $("#f-tags").value = parseList(t.join(",")).join(", "); renderTagHelpers(); });
    $("#collection-suggest").addEventListener("click", (e) => { const chip = e.target.closest("[data-add-collection]"); if (!chip)
        return; const t = parseList($("#f-collections").value); t.push(chip.dataset.addCollection); $("#f-collections").value = parseList(t.join(",")).join(", "); renderTagHelpers(); });
    ...
    $("#detail-body").addEventListener("click", (e) => {
        ...
        if (act === "log")
            ...
            openRecommendModal({ title: book.title, author: book.author, category: (book.tags || [])[0] || "", isbn: book.isbn, cover: book.coverUrl });
        else if (act === "toggle-owned") {
            book.owned = !book.owned;
            commit();
            toast("🏠", book.owned ? "Added to your home shelf" : "Removed from your home shelf", book.title);
        }
            ...
            openLendModal(book);
        else if (act === "lend-return") {
            const who = book.lentTo;
            book.lentTo = "";
            book.lentAt = null;
            commit();
            toast("↩", "Welcome back!", `“${book.title}” returned from ${who}`);
        }
        else if (act === "start") {
            book.status = "reading";
            book.startedAt = new Date().toISOString();
        ...
        else if (act === "share-card") {
            shareBookCard(book);
        }
        else if (act === "export-md") {
            downloadText(slugify(book.title) + "-journal.md", bookMarkdown(book));
            toast("⬇️", "Journal exported", book.title + " as Markdown");
        }
        else if (act === "edit-log") {
            const lg = book.logs.find((x) => x.id === btn.dataset.log);
            if (lg)
                openLogModal(book, lg);
        }
        else if (act === "del-log") {
    ...
    $("#year-body").addEventListener("click", (e) => {
        ...
        if (act) {
            ...
            if (act.dataset.yrAction === "image")
                shareYearCard(y);
            else {
                downloadText("my-" + y + "-in-books.md", yearMarkdown(y));
                toast("⬇️", "Year journal exported", y + " as Markdown");
            }
            return;
        }
        const b = e.target.closest("[data-year-nav]");
        if (b && !b.disabled)
            openYearReview(Number(b.dataset.yearNav));
    });
    ...
    $("#btn-giftlist").addEventListener("click", openGiftList);
    $("#btn-giftlist-image").addEventListener("click", shareGiftListImage);
    $("#btn-giftlist-link").addEventListener("click", shareGiftListLink);
    $("#month-body").addEventListener("click", (e) => {
        const b = e.target.closest("[data-month-nav]");
        if (b && !b.disabled) {
            const [y, m] = b.dataset.monthNav.split("-").map(Number);
            openMonthlyRecap(y, m);
        }
    });
    ...
    $("#btn-ereader").addEventListener("click", () => {
        if (EReader)
            ...
            toast("⚠️", "eReader unavailable", "The reader script didn't load.");
    });
    // Data menu
    $("#btn-export").addEventListener("click", exportJSON);
    $("#btn-import").addEventListener("click", () => $("#import-input").click());
    $("#import-input").addEventListener("change", (e) => { if (e.target.files[0])
        importJSON(e.target.files[0]); e.target.value = ""; });
    $("#btn-goodreads").addEventListener("click", () => $("#goodreads-input").click());
    $("#goodreads-input").addEventListener("change", (e) => { if (e.target.files[0])
        importGoodreads(e.target.files[0]); e.target.value = ""; });
    $("#btn-connect-file").addEventListener("click", connectFile);
    $("#btn-theme").addEventListener("click", toggleTheme);
    // Settings / data safety
    $("#btn-settings").addEventListener("click", openSettings);
    $("#storage-status").addEventListener("click", openSettings);
    $("#storage-status").addEventListener("keydown", (e) => { if (e.key === "Enter" || e.key === " ") {
        e.preventDefault();
        openSettings();
    } });
    ...
    $("#clubs-body").addEventListener("click", (e) => {
        ...
        if (react) {
            toggleReaction(react.dataset.comment, react.dataset.react);
            return;
        }
        const open = e.target.closest("[data-club-open]");
        if (open) {
            openClub(open.dataset.clubOpen);
            return;
        }
        if (e.target.closest("[data-club-back]")) {
            renderClubsListScreen();
            return;
        }
        const leave = e.target.closest("[data-club-leave]");
        if (leave) {
            if (confirm("Leave this club? You can rejoin later with the code."))
                clubApi("/" + leave.dataset.clubLeave + "/leave", { method: "POST" }).then(() => { toast("👋", "Left the club", ""); renderClubsListScreen(); });
            return;
        }
        const share = e.target.closest("[data-club-share]");
        if (share && share.dataset.clubShare) {
            shareClubInvite(share.dataset.clubShare, share.dataset.clubShareTitle || "");
            return;
        }
        const copy = e.target.closest("[data-club-copy]");
        if (copy && copy.dataset.clubCopy) {
            try {
                navigator.clipboard.writeText(copy.dataset.clubCopy);
            }
            catch (e2) { /* ignore */ }
            toast("📋", "Invite code copied", copy.dataset.clubCopy);
            return;
        }
    });
    $("#clubs-body").addEventListener("submit", (e) => {
        ...
        if (e.target.id === "club-create-form") {
            const title = $("#club-book-title").value.trim();
            if (!title)
                return;
            clubApi("", { method: "POST", body: JSON.stringify({ bookTitle: title, bookAuthor: $("#club-book-author").value.trim(), displayName: clubDisplayName() }) })
                .then(({ res, data }) => { if (res.ok && data.clubId) {
                toast("👥", "Club created", "Invite code " + data.joinCode);
                openClub(data.clubId);
            }
            else
                toast("⚠️", "Couldn't create the club", (data && data.error) || ""); });
        }
        else if (e.target.id === "club-join-form") {
            const code = $("#club-join-code").value.trim().toUpperCase();
            if (!code)
                return;
            joinClubByCode(code);
        }
    ...
    $("#clubs-body").addEventListener("input", (e) => { if (e.target.id === "club-progress-range") {
        const v = $("#club-my-pct");
        if (v)
            v.textContent = e.target.value;
    } });
    $("#clubs-body").addEventListener("change", (e) => { if (e.target.id === "club-progress-range")
        setClubProgress(Number(e.target.value) || 0); });
    ...
    $("#community-body").addEventListener("click", (e) => {
        if (e.target.closest("[data-community-signin]")) {
            closeModals();
            openAuthModal("login");
            return;
        }
        if (e.target.closest("[data-community-showread]")) {
            communityHideRead = false;
            const cb = $("#community-hide-read");
            if (cb)
                cb.checked = false;
            drawCommunity();
            return;
        ...
        if (vote)
            voteRec(id, Number(vote.dataset.vote));
    });
    ...
    $("#doctor-body").addEventListener("click", async (e) => {
        ...
        if (mg) {
            const g = duplicateGroups().find((x) => x.key === mg.dataset.doctorMerge);
            if (g)
                mergeDuplicateGroup(g.books);
            return;
        }
        ...
        if (fix === "cover") {
            fx.disabled = true;
            fx.textContent = "Finding…";
            try {
                const url = await findCoverFor(book);
                if (url) {
                    book.coverUrl = url;
                    commit();
                    toast("🖼", "Cover found", book.title);
                    renderShelfDoctor();
                }
                else {
                    toast("🔍", "No cover found", "Try editing and pasting one.");
                    fx.disabled = false;
                    fx.textContent = "🔍 Find cover";
                }
            }
            catch (err) {
                toast("⚠️", "Lookup failed", "Check your connection.");
                fx.disabled = false;
                fx.textContent = "🔍 Find cover";
            }
            return;
        ...
        if (fix === "genres") {
            ...
            try {
                ...
                if (picks.length) {
                    ...
                    toast("🔍", "No genres found", "Add a couple by hand.");
                    fx.disabled = false;
                    fx.textContent = "✨ Fetch genres";
                }
            }
            catch (err) {
                toast("⚠️", "Couldn't fetch genres", "Check your connection.");
                fx.disabled = false;
                fx.textContent = "✨ Fetch genres";
            }
            return;
    ...
    $("#onboard-modal").addEventListener("click", (e) => {
        ...
        closeModals();
        if (act === "goodreads")
            $("#goodreads-input").click();
        else if (act === "add")
            openBookModal({ status: "reading" });
        else if (act === "goal")
            switchView("goals");
        else if (act === "install")
            promptInstall();
    ...
    // Modal close
    $$(".modal-backdrop").forEach((m) => m.addEventListener("click", (e) => { if (e.target === m)
        closeModals(); }));
    document.addEventListener("click", (e) => { if (e.target.closest("[data-close-modal]"))
        closeModals(); });
    document.addEventListener("keydown", (e) => {
        ...
        closeModals();
        if (!hadOpenModal && activeView === "book")
            goBackFromBook();
    });
    ...
    window.addEventListener("popstate", () => {
        ...
        if (b)
            openBookPage(b, { push: false });
        else
            closeBookPage();
    });
    if (!supportsFS)
        $("#btn-connect-file").style.display = "none";
    ...
    if (deepBook)
        openBookPage(deepBook, { push: false });
    else if (deepLink)
        histCleanHash();
