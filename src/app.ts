...
function shiftDay(dayTs: number, days: number) {
...
function dayspan(fromTs: number, toTs: number) { return Math.round((toTs - fromTs) / DAY); }
...
function allTags(): string[] { return derived("allTags", () => uniqueValues((b) => b.tags)); }
function allCollections(): string[] { return derived("allCollections", () => uniqueValues((b) => b.collections)); }
...
function derived<T>(key: string, compute: () => T): T {
  if (derivedEpoch !== dataEpoch) { derivedCache.clear(); derivedEpoch = dataEpoch; }
  if (!derivedCache.has(key)) derivedCache.set(key, compute());
  return derivedCache.get(key) as T;
}
...
async function persist() {
  try {
    ...
    if (storageFull) { storageFull = false; renderStorageStatus(); }
  } catch (e: any) {
    console.warn(e);
    const quota = e && (e.name === "QuotaExceededError" || e.name === "NS_ERROR_DOM_QUOTA_REACHED" || e.code === 22);
    if (!storageFull) {
      storageFull = true;
      toast("⚠️", quota ? "This device is out of storage" : "Couldn't save on this device",
        quota ? "Your latest changes aren't saved locally. Export a backup, then remove some ePubs or books."
...
function passwordStrength(password: string) {
  ...
  if (passwordProblem(password)) return { score: 1, label: "too weak" };
  let score = 2;
  const classes = [/[a-z]/, /[A-Z]/, /[0-9]/, /[^a-zA-Z0-9]/].filter((re) => re.test(password)).length;
  if (password.length >= 14 || (password.length >= 12 && classes >= 3)) score = 3;
  if (password.length >= 18 || (password.length >= 14 && classes >= 3)) score = 4;
  return { score, label: ["", "too weak", "okay", "good", "strong"][score] };
}
function renderStrength(prefix: string, password: string) {
...
function openAuthModal(mode: "login" | "register") {
  ...
  $<HTMLInputElement>("#auth-name").value = ""; $<HTMLInputElement>("#auth-email").value = ""; $("#auth-error").textContent = "";
  ...
  serverCaps().then((caps) => { const b = $("#auth-forgot"); if (b) b.hidden = authMode !== "login" || !caps.passwordReset; });
...
function setAuthMode(mode: "login" | "register") {
  ...
  $<HTMLInputElement>("#auth-password").autocomplete = mode === "register" ? "new-password" : "current-password";
  $("#auth-error").textContent = "";
  if (mode === "login") { $<HTMLInputElement>("#auth-password2").value = ""; renderStrength("auth", ""); }
  else serverCaps().then((caps) => { const b = $("#auth-forgot"); if (b && authMode === "register") b.hidden = true; else if (b) b.hidden = !caps.passwordReset; });
}
/** Never leave a password sitting in a detached DOM node after the modal closes. */
function clearPasswordFields(scope: string) {
  const form = document.querySelector(scope);
  if (!form) return;
  form.querySelectorAll<HTMLInputElement>('input[type="password"]').forEach((i) => { i.value = ""; i.type = "password"; });
  form.querySelectorAll<HTMLButtonElement>(".pw-reveal").forEach((b) => { b.classList.remove("on"); b.textContent = "👁️"; });
...
let serverCapsCache: Promise<{ passwordReset: boolean }> | null = null;
function serverCaps() {
  if (!serverCapsCache) {
...
async function onAuthSubmit(e: SubmitEvent) {
  ...
  if (authMode === "register") {
    if (!creds.email) { err.textContent = "Please enter your email address."; return; }
    if (!creds.fullName) { err.textContent = "Please enter your full name."; return; }
    const bad = passwordProblem(creds.password, creds.email, creds.fullName);
    if (bad) { err.textContent = bad; return; }
    if (creds.password !== $<HTMLInputElement>("#auth-password2").value) { err.textContent = "The two passwords don't match."; return; }
...
function openForgotModal(prefillEmail?: string) {
  closeModals();
  ...
  $<HTMLButtonElement>("#reset-ask-submit").disabled = false;
  $<HTMLInputElement>("#reset-email").value = prefillEmail || "";
  clearPasswordFields("#reset-set-form");
  showModal("reset-modal");
  setTimeout(() => $<HTMLInputElement>("#reset-email").focus(), 50);
}
function openResetModal(token: string) {
  ...
  clearPasswordFields("#reset-set-form");
  showModal("reset-modal");
  setTimeout(() => $<HTMLInputElement>("#reset-password").focus(), 50);
}
async function onForgotSubmit(e: SubmitEvent) {
  e.preventDefault();
  ...
  const submit = $<HTMLButtonElement>("#reset-ask-submit"); submit.disabled = true;
  try {
    const { res, data } = await apiFetch("/api/password/forgot", { method: "POST", body: JSON.stringify({ email }) });
    if (!res.ok) { err.textContent = (data && data.error) || "Something went wrong. Please try again."; submit.disabled = false; return; }
    ...
    if (data && data.devResetLink) { done.textContent += " (dev: " + data.devResetLink + ")"; }
  } catch (e2: any) {
    err.textContent = "Couldn't reach the server. Check your connection and try again.";
    submit.disabled = false;
  }
}
async function onResetSubmit(e: SubmitEvent) {
  e.preventDefault();
  const err = $("#reset-set-error");
  err.textContent = "";
  const pw = $<HTMLInputElement>("#reset-password").value;
  if (!pendingResetToken) { err.textContent = "That reset link is invalid. Please request a new one."; return; }
  const bad = passwordProblem(pw);
  if (bad) { err.textContent = bad; return; }
  if (pw !== $<HTMLInputElement>("#reset-password2").value) { err.textContent = "The two passwords don't match."; return; }
  const submit = $<HTMLButtonElement>("#reset-set-submit"); submit.disabled = true;
  try {
    const { res, data } = await apiFetch("/api/password/reset", { method: "POST", body: JSON.stringify({ token: pendingResetToken, newPassword: pw }) });
    if (!res.ok) { err.textContent = (data && data.error) || "Something went wrong. Please try again."; return; }
    pendingResetToken = null;
    saveAuth({ token: data.token, user: data.user });
    saveSyncBase(null);
    clearPasswordFields("#reset-set-form");
...
function openPasswordModal() {
  closeAccountMenu();
  if (!auth) { openAuthModal("login"); return; }
  clearPasswordFields("#password-form");
  $("#password-error").textContent = "";
  showModal("password-modal");
  setTimeout(() => $<HTMLInputElement>("#pw-current").focus(), 50);
}
async function onPasswordSubmit(e: SubmitEvent) {
  ...
  if (!current) { err.textContent = "Please enter your current password."; return; }
  const bad = passwordProblem(next, auth && auth.user ? auth.user.email : "", auth && auth.user ? auth.user.fullName : "");
  if (bad) { err.textContent = bad; return; }
  if (next === current) { err.textContent = "That's the password you already have."; return; }
  if (next !== $<HTMLInputElement>("#pw-new2").value) { err.textContent = "The two passwords don't match."; return; }
  const submit = $<HTMLButtonElement>("#password-submit"); submit.disabled = true;
  try {
    ...
    if (res.status === 401 && data && data.error && /current password/i.test(data.error)) { err.textContent = data.error; return; }
    if (res.status === 401) { handleAuthExpired(); closeModals(); return; }
    if (!res.ok) { err.textContent = (data && data.error) || "Something went wrong. Please try again."; return; }
...
function streakFromDays(days: Set<number>) {
  if (days.size === 0) return { current: 0, longest: 0 };
  const sorted = Array.from(days).sort((a, b) => a - b);
  let longest = 1, run = 1;
  for (let i = 1; i < sorted.length; i++) {
    if (sorted[i] === shiftDay(sorted[i - 1], 1)) { run++; longest = Math.max(longest, run); }
    else run = 1;
  }
...
const VIEW_RENDERERS: Record<string, () => void> = {
...
const staleViews = new Set<string>(Object.keys(VIEW_RENDERERS));
function renderActiveView() {
  const fn = VIEW_RENDERERS[activeView];
  if (!fn) return; // "book" and "community" render through their own paths
...
function render() {
  ...
  if (PERF) console.log("[perf] render " + Math.round(performance.now() - perfT0) + "ms · " + activeView + " · " + state.books.length + " books");
}
...
function moodMatchLine() {
  ...
  const tally: Record<string, number> = {};
  recent.forEach((l) => { if (l.mood) tally[l.mood] = (tally[l.mood] || 0) + 1; });
...
function mergeShelfOrder(prevOrder: string[], visible: string[], knownIds: string[]): string[] {
  ...
  for (const id of prev) {
    if (onShelf.has(id)) {
      if (!placed) { out.push(...visible); placed = true; } // first slot the run occupied
      continue;
    }
    out.push(id);
  }
  if (!placed) out.push(...visible); // nothing was ordered yet, or none of these were
  return out;
}
...
function dailyItems(perDay: Record<number, number>, endDay?: number): ChartItem[] {
...
function refreshDetail(force?: boolean) {
  ...
  if (!force && active && $("#detail-body").contains(active) && /^(input|textarea|select)$/i.test(active.tagName)) return;
...
// ---------------------------------------------------------------------------
function duplicateGroups(): { key: string; books: Book[] }[] {
...
async function backfillGenres() {
  if (genreBackfillBusy) return;
  if (navigator.onLine === false) { toast("📡", "You're offline", "Genre lookup needs a connection."); return; }
  genreBackfillBusy = true;
  const btn = $<HTMLButtonElement>("#btn-doctor-genres");
  try {
    const todo = state.books.filter((b) => !(b.tags && b.tags.length));
    if (!todo.length) { toast("🎉", "Nothing to do", "Every book already has genres."); return; }
    let filled = 0;
    for (let i = 0; i < todo.length; i++) {
      const b = todo[i];
      if (!state.books.includes(b)) continue; // a sync pull may have replaced the list
      if (btn) btn.textContent = `Fetching… ${i + 1}/${todo.length}`;
      try {
        const picks = await fetchGenresForBook(b);
        if (picks.length) {
          const have = new Set((b.tags || []).map((x) => x.toLowerCase()));
          picks.forEach((p) => { if (!have.has(p.toLowerCase())) { have.add(p.toLowerCase()); b.tags.push(p); } });
          filled++;
          renderShelfDoctor(); // keep the "No genres yet" list shrinking as we go
        }
      } catch (e: any) { /* skip this one, keep going */ }
      await sleep(350);
    }
    if (filled) commit();
    toast("🏷️", filled ? "Genres added" : "No genres found", filled ? `Filled in ${filled} book${filled === 1 ? "" : "s"}.` : "Try adding a few by hand.");
  } finally { genreBackfillBusy = false; }
}
...
async function openClubWs(clubId: string) {
  closeClubWs();
  if (!("WebSocket" in window) || !SYNC_API || !auth) return;
  try {
    const { res, data } = await clubApi("/" + encodeURIComponent(clubId) + "/ws-ticket", { method: "POST" });
    if (!res.ok || !data || !data.ticket) return; // no realtime; polling covers it
    if (currentClubId !== clubId) return;         // moved on while the ticket was in flight
...
function safeCoverUrl(url: unknown) {
  try {
    const u = new URL(String(url || ""));
    return u.protocol === "https:" && COVER_HOSTS.test(u.hostname) ? u.href : "";
  } catch (e: any) { return ""; } // not a URL at all
}
...
function savedTimer(): { start: number; bookId: string } | null {
  try { const v = JSON.parse(localStorage.getItem(TIMER_KEY) || "null"); return v && typeof v.start === "number" ? v : null; } catch (e: any) { return null; }
}
function paintTimer() {
  const read = $("#timer-read");
  if (!read || timerStart == null) return;
...
function runTimer(startAt: number) {
...
function resetTimer(clearSaved = true) {
  ...
  if (clearSaved) { try { localStorage.removeItem(TIMER_KEY); } catch (e: any) { /* ignore */ } }
...
function restoreTimer(bookId: string) {
  const saved = savedTimer();
  if (saved && saved.bookId === bookId && Date.now() - saved.start < TIMER_MAX_MS) runTimer(saved.start);
...
function toggleTimer() {
  if (timerStart) {
    ...
    try { localStorage.setItem(TIMER_KEY, JSON.stringify({ start, bookId: $<HTMLInputElement>("#log-book-id").value })); } catch (e: any) { /* ignore */ }
    runTimer(start);
  }
}
...
async function connectFile() {
  ...
  try {
    ...
    if (text.trim()) { try { state = normalize(JSON.parse(text)); invalidateDerived(); knownBadges = new Set(); } catch (e: any) { /* keep */ } }
    await persist();
    render();
    knownBadges = new Set(computeBadges().filter((b) => b.unlocked).map((b) => b.id));
...
function switchView(view: string) {
  ...
  if (staleViews.has(view)) renderActiveView(); // it missed the changes while hidden
...
function init() {
  ...
  $("#detail-body").addEventListener("click", (e) => {
    ...
    else if (act === "del-log") { const lg = book.logs.find((x) => x.id === btn.dataset.log); if (lg && confirm(`Delete this log of ${num(lg.pages)} pages?`)) { book.logs = book.logs.filter((x) => x.id !== lg.id); commit(); toast("🗑", "Log removed", book.title); } }
    else if (act === "del-quote") { book.quotes = (book.quotes || []).filter((q) => q.id !== btn.dataset.quote); commit(); }
    else if (act === "del-journal") { book.journal = (book.journal || []).filter((j) => j.id !== btn.dataset.journal); commit(); }
    else if (act === "del-char") { book.characters = (book.characters || []).filter((c) => c.id !== btn.dataset.char); commit(); }
    else if (act === "del-vocab") { book.vocab = (book.vocab || []).filter((v) => v.id !== btn.dataset.vocab); commit(); }
  });
  $("#detail-body").addEventListener("submit", (e) => {
    ...
    if ((e.target as HTMLElement).id === "quote-form") {
      ...
      commit(); refreshDetail(true); // the field still has focus; force past the guard
  ...
  $<HTMLButtonElement>("#btn-doctor-genres").addEventListener("click", async (e) => {
    const btn = e.currentTarget as HTMLButtonElement; btn.disabled = true; btn.textContent = "Fetching…";
    await backfillGenres();
    btn.disabled = false; btn.textContent = "🏷️ Fetch all missing genres";
    renderShelfDoctor();
  });
  ...
  if (syncEnabled()) {
    ...
    $<HTMLButtonElement>("#am-password").addEventListener("click", openPasswordModal);
    ...
    $<HTMLButtonElement>("#auth-forgot").addEventListener("click", () => openForgotModal($<HTMLInputElement>("#auth-email").value.trim()));
    $<HTMLFormElement>("#reset-ask-form").addEventListener("submit", onForgotSubmit);
    $<HTMLFormElement>("#reset-set-form").addEventListener("submit", onResetSubmit);
    $<HTMLFormElement>("#password-form").addEventListener("submit", onPasswordSubmit);
    // Live strength read-out on every field that takes a NEW password.
    ([["#auth-password", "auth"], ["#reset-password", "reset"], ["#pw-new", "pw"]] as const).forEach(([sel, prefix]) => {
      $<HTMLInputElement>(sel).addEventListener("input", (e) => renderStrength(prefix, (e.target as HTMLInputElement).value));
    });
    ...
    document.addEventListener("click", (e) => {
      const b = (e.target as HTMLElement).closest<HTMLButtonElement>(".pw-reveal");
      if (!b || !b.dataset.reveal) return;
